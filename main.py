...
async def menu_msg(num, message, state):
    ...
    await state.update_data(setting_id=setting.id)
    try:
        start_kb = load_kb(setting.start_kb)
        await bot.copy_message(message.from_user.id, setting.start_from_user_id, setting.start_msg_id, reply_markup=start_kb)
    except:
...
@dp.message_handler(lambda msg: msg.text.isdecimal(), is_admin=True, state=StartMailingStates.num)
async def _settings_start(message: Message, state: FSMContext):
...
async def send_start_msg(send_to, setting: Setting):
    # if chat_id != 0:
...
async def process_update(chat_member: types.ChatJoinRequest):
    ...
    setting = Setting.get_or_none(link=link)
    if setting is None:
        await notify_admins(f"Неизвестная ссылка: {link}")
        return
    if setting.send_start:
        await send_start_msg(send_to, setting)
